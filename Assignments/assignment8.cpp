...
int main() {
	...
	while (1) {
		...
		if (press == 78 || press == 110) {//n,N
			...
			blue_rectangle = Rect(faces[i].x, faces[i].y-20, faces[i].width, faces[i].height+40);
			if (t_press == 1) {
			...
			Point tr(faces[i].x, faces[i].y);
			rectangle(image, lb, tr, Scalar(0, 255, 0), 3, 4, 0);
			blue_rectangle = Rect(faces[i].x, faces[i].y-15, faces[i].width, faces[i].height+40);
			...
			Point tr(faces[i].x, faces[i].y);
			rectangle(image, lb, tr, Scalar(0, 255, 0), 3, 4, 0);
			blue_rectangle = Rect(faces[i].x, faces[i].y-10, faces[i].width, faces[i].height+20);
