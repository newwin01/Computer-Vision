...
int main(){
    ...
    orbF->detectAndCompute(img, noArray(), keypoints2, descriptors2); 
    for(int count=0;count<DB_str.size();count++){
        ...
        orbF->detectAndCompute(DB_img, noArray(), keypoints1, descriptors1);
        k = 2;
        matcher.knnMatch(descriptors2, descriptors1, matches, k);
        nndr = 0.6f;
